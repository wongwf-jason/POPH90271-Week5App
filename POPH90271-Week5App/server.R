  ...
  output$M3_out_plot <- renderPlotly({
    ...
    withProgress(message = "Specifying initial states and parameters...", value = 0.1, {
      ...
      incProgress(message = "Solving ordinary differential equations...",
                  detail = HTML("Cholera is being transmitted \uD83E\uDDA0! This should take no longer than ~30 seconds \uD83D\uDE47, Unless you're using brute force!"), amount = 0.2)
...
#     solutions_list <- mapply(solve_model, state_list, parameters_list, SIMPLIFY = FALSE) # !Use this line with upgraded code for uncerain states! 
  ...
  output$M3_out_grid_search_samples <- renderUI(p(HTML(paste("With <b>2</b> uncertain parameters, there are", input$M3_sample_n, "<sup>2</sup>", "= <b>", input$M3_sample_n^2, "</b> combinations across the parameter space."))))
